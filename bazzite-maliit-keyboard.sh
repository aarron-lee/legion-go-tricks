...
cat << EOF > "$HOME/.config/plasma-workplace/env/immodule_temp_fix.sh"
#!/bin/bash
unset GTK_IM_MODULE
unset QT_IM_MODULE
EOF

sudo chcon -u system_u -r object_r --type=bin_t "$HOME/.config/plasma-workplace/env/immodule_temp_fix.sh"
